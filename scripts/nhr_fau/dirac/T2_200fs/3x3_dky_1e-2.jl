...
global_logger(make_teelogger(
	joinpath(ENV["WORK"],"dirac/T2_200fs/zeta-M-sweep1/3x3_dky_1e-2"),
	"slurmid="*ENV["SLURM_JOB_ID"]*"dky_1e-2"))


const files = load_hdf5_files(joinpath(
	ENV["WORK"],
	"dirac/T2_200fs/zeta-M-sweep1/3x3_kxmax_1e-3"))
...
const newsims  = [@set s.numericalparams.dky = 0.1 for s in kymaxsims]
