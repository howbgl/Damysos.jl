...
function make_simulation(
	...
	id = "M=$(M)_zeta=$(ζ)_dt_vs_rtol"

	return Simulation(l, df, pars, obs, us, id, datapath, plotpath)
end

const gpuname = CUDA.name(CUDA.device())
const path = joinpath(@__DIR__, "dt_vs_rtol_on_$gpuname")
const altpath = joinpath(pwd(), "dt_vs_rtol_on_$gpuname")

const parpairs = [(z,m) for m in [0.1,1.,10.] for z in [0.1,1.,10.]]
const sims = [make_simulation(m,z,1.0,1.0;datapath=path) for (z,m) in parpairs]
...
const logger = make_teelogger(path,"dt-vs-rtol")
...
CSV.write(joinpath(path,"rtol_vs_runtime_on_CPU.csv"),data)
