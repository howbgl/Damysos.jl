...
pars    = NumericalParams2d(0.01,0.01,4,0.2,0.5,-5df.σ)
obs     = [Velocity(h)]
sim     = Simulation(h,df,pars,obs,us,2)
ens     = parametersweep(sim,sim.numericalparams,:kymax,[0.5,1.0,2.0,3.0])
logger  = FileLogger(joinpath("logs","dirac2d_50meV_dt_$(now()).log"))
