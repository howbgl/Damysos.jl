...
function makesim(ζ,M;datapath = pwd(), plotpath = datapath)
    ...
    dkx = 8pars_row.dkx
	dky = pars_row.dky
	dt = 0.003 # ensures rtol ≈ 0.05

	subdir    = "dirac/T2_50fs/zeta-M-sweep1/3x3_dkx_1e-2"		
...
global_logger(make_teelogger(
	joinpath(ENV["WORK"],"dirac/T2_50fs/zeta-M-sweep1/3x3_dkx_1e-2"),
	"slurmid="*ENV["SLURM_JOB_ID"]*"dkx_1e-2"))
