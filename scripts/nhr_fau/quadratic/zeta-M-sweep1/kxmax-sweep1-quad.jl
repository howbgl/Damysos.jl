...
# const subdir 	= "quadratic/zeta-M-sweep1/dt_1e-6_dkx_1e-2_dky_1e-2_kxmax_1e-2"
const subdir 	= "quadratic/zeta-M-sweep1/new_dt_1e-4_dkx_1e-3_kxmax_1e-4"
const files		= load_hdf5_files(joinpath(ENV["WORK"],"quadratic/zeta-M-sweep1/new_dt_1e-4_dkx_1e-3"))
const path 		= files[id]
...
const newpath 	= joinpath(
	...
	subdir,
	m.match,
	"convergencetest_LinearTest_dt_1e-4_dkx_1e-3_kxmax_1e-4.hdf5")

const test 		= ConvergenceTest(
	path,
	LinearCUDA();
	resume = false,
	method = PowerLawTest(:kxmax,1.1),
	atolgoal = 1e-12,
	rtolgoal = 1e-3,
