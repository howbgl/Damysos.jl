...
abstract type Observables{T}            <: SimulationComponent{T} end
...
struct Velocity{T<:Real} <: Observables{T}
    
end

function calc_obs(sim::Simulation{T},vel::Velocity{T},sol) where {T<:Real}
    
end

function calc_obs(sim::Simulation{T}) where {T<:Real}
