using Damysos,Unitful,LoggingExtras,Dates,Formatting,TerminalLoggers,Dagger
...
function make_n_runtest(s)
  method      = SequentialTest(
      ...
      run!(test,40,4*60*60,savealldata=false)
end
...
const sims     = [make_system(
      z,
      m,
      "hhgjl/inter-intra-cancellation/multiphoton/";
      rtol=1e-4,
...
@info "Just run once before mapping for compilation"
make_n_runtest(sims[1])
