...
function runbench!(sims,fns,solver)
    ...
    for (s,f) in zip(sims,fns)
        ...
        times =  [@elapsed run!(s,f,solver;savedata=false,saveplots=false) for _ in 1:5]
...
const path = joinpath(@__DIR__, "kpoint_cpu")
const sims = [make_demo_simulation(dkx,path) for dkx in exp10.(1:-0.04:-0.6)]
const nks = [getnkx(s)*getnky(s) for s in sims]
const solver = LinearChunked(1_000)
const fns = [define_functions(s,solver) for s in sims]

const logger = make_teelogger(@__DIR__, "kpoint_cpu")
...
CSV.write(joinpath(path,"nk_vs_runtime_on_CPU.csv"),data)
