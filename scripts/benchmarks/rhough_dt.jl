...
path(ζ) = joinpath(ENV["WORK"], "rhough_dt/1e-8/zeta=$(ζ)")
altpath(ζ) = joinpath(ENV["HPCVAULT"], "rhough_dt/1e-8/zeta=$(ζ)")
const sims = [make_simulation(m,ζ,path(ζ)) for m in Ms for ζ in ζs]
const tests = [make_dt_test(s) for s in sims]

const logger = make_teelogger(
	joinpath(ENV["WORK"],"rhough_dt/1e-8"), 
