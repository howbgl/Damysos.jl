...
function make_simulation(M = 0.5, ζ = 4.0, datapath = "~/data", plotpath = datapath)
	...
	id = "M=$(M)_zeta=$(ζ)_rough_dkx_1e-4"
...
path(ζ) = joinpath(ENV["WORK"], "rhough_dkx/1e-4/zeta=$(ζ)")
altpath(ζ) = joinpath(ENV["HPCVAULT"], "rhough_dkx/1e-4/zeta=$(ζ)")
const sims = [make_simulation(m,ζ,path(ζ)) for m in Ms for ζ in ζs]
const tests = [make_dkx_test(s) for s in sims]

const logger = make_teelogger(
	joinpath(ENV["WORK"],"rhough_dkx/1e-4"), 
