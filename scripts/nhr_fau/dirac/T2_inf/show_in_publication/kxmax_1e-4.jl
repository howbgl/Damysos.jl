...
global_logger(make_teelogger(
	joinpath(ENV["WORK"],"dirac/T2_inf/show_in_publication/kxmax_1e-4"),
	"slurmid="*ENV["SLURM_JOB_ID"]*"kxmax_1e-4"))
...
const tests = [ConvergenceTest(f,LinearCUDA();
	resume = false,
	method = PowerLawTest(:kxmax,1.4),
	rtolgoal = 1e-4,
	atolgoal = 1e-6,
	maxtime = u"60minute",
	maxiterations = 12,
	path = replace(f,"dkx_1e-4" => "kxmax_1e-4")) for f in files]
