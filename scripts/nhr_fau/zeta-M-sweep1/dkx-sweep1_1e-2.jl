...
const subdir 	= "zeta-M-sweep1/dt_1e-4_dkx_1e-2"

global_logger(make_teelogger(joinpath(ENV["WORK"],subdir),"slurmid="*ENV["SLURM_ARRAY_TASK_ID"]))

files 			= load_hdf5_files(joinpath(ENV["WORK"],"zeta-M-sweep1/dt_1e-4"))
const path 		= files[id]
const newpath 	= replace(path,"dt_1e-4" => "dt_1e-4_dkx_1e-2")

const t = ConvergenceTest(path,LinearCUDA();resume=false)
