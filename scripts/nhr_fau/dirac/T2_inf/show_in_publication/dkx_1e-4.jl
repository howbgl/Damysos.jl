...
global_logger(make_teelogger(
	joinpath(ENV["WORK"],"dirac/T2_inf/show_in_publication/dkx_1e-4"),
	"slurmid="*ENV["SLURM_JOB_ID"]*"dkx_1e-4"))


const files = load_hdf5_files(joinpath(
	ENV["WORK"],
	"dirac/T2_inf/show_in_publication/dt_1e-4"))
...
const tests = [ConvergenceTest(f,LinearCUDA();
	...
	maxtime = u"60minute",
	maxiterations = 12,
	path = replace(f,"dt_1e-4" => "dkx_1e-4")) for f in files]
