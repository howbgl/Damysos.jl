...
pars    = NumericalParams2d(0.1,0.1,5,2,0.5,-5df.σ)
obs     = [Velocity(h)]
sim     = Simulation(h,df,pars,obs,us,2)
ens     = parametersweep(sim,sim.numericalparams,:kxmax,[4.,5.,6.])
logger  = FileLogger(joinpath("logs","dirac2d_kxmax_$(now()).log"))

global_logger(logger)
@info "$(now())\nOn $(gethostname()):"

results,time,rest... = @timed run_simulation!(ens;kyparallel=false)
