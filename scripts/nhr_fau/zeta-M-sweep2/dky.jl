...
function makesim(M = 0.5, ζ = 4.0, dt = 0.1, dkx = 0.1, dky = 10dkx; 
	datapath = "~/data", 
	plotpath = datapath)
...
function maketest(sim::Simulation,altpath::String=pwd())
	...
	method = PowerLawTest(:dky,0.5)
	atolgoal = 1e-12
	rtolgoal = 1e-5 
	maxtime = u"20*60minute"
	maxiter = 32 
...
const start = CSV.File(joinpath(
	ENV["WORK"],
	"zeta-M-sweep2/dky3/results.csv"))
