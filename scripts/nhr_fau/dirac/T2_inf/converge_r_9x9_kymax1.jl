...
const dir 	 = joinpath(ENV["WORK"],"dirac/T2_inf/converge_r_9x9_dky1") # start with lowest kmax, is sufficient
const files  = load_hdf5_files(dir)
const id 	 = parse(Int, ENV["SLURM_ARRAY_TASK_ID"])
const file 	 = files[id]
const path 	 = replace(file,"dky1" => "kymax1")
const test   = ConvergenceTest(file,LinearCUDA(10_000);
	resume = false,
	method = PowerLawTest(:kymax,1.2),
	path = path,
