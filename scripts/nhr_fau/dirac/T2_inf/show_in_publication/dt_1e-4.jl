...
const dir 		= joinpath(ENV["WORK"],"dirac/T2_inf/show_in_publication/dt_1e-4")
const paths 	= [joinpath(dir,"zeta=$(c[1])_M=$(c[2])") for c in coords]
const tests		= [maketest(c[1],c[2],
	joinpath(p,"dt_1e-4.hdf5")) for (c,p) in zip(coords,paths)] 

global_logger(make_teelogger(dir,"slurmid="*ENV["SLURM_JOB_ID"]*"dt_1e-4"))
