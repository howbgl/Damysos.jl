...
global_logger(make_teelogger(
	joinpath(ENV["WORK"],"dirac/T2_100fs/zeta-M-sweep1/3x3_dky_1e-3"),
...
const newsims  = [@set s.numericalparams.dky = s.numericalparams.kymax/50. for s in kymaxsims]
...
const tests = [ConvergenceTest(s,LinearCUDA();
	...
	maxiterations = 12,
	path = replace(f,"dkx_1e-3" => "dky_1e-3")) for (s,f) in zip(newsims,newpaths)]
