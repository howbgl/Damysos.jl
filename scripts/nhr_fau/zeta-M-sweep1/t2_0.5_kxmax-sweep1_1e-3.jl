...
const subdir 	= "zeta-M-sweep1/t2_0.5_dt_1e-4_dkx_1e-2_dky_1e-2_kxmax_1e-3"

global_logger(make_teelogger(joinpath(ENV["WORK"],subdir),"slurmid="*ENV["SLURM_ARRAY_TASK_ID"]))

files 			= load_hdf5_files(joinpath(ENV["WORK"],"zeta-M-sweep1/dt_1e-4_dkx_1e-2_dky_1e-2"))
const path 		= files[id]
...
const newpath 	= joinpath(
	...
	subdir,
	m.match,
	"convergencetest_PowerLawTest_t2_0.5_dt_1e-4_dkx_1e-2_dky_1e-2_kxmax_1e-3.hdf5")
const oldsim 	= loadlast_testsim()
