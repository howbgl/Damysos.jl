...
function make_n_runtest(s)
      method      = SequentialTest(
            ...
            PowerLawTest(:dky,0.5),
            LinearTest(:kxmax,50.),
            LinearTest(:kymax,50.)])
      test        = ConvergenceTest(s,method,1e-12,1e-4)

      run!(test,100,2*60*60,savealldata=false,sequentialsim=false)
end


const sim = make_system(3.0,6.0,"hhgjl/inter/intra/cancellation/multiphoton/right";
