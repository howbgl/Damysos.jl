...
const subdir 	= "zeta-M-sweep1/17x17_dt_1e-4"
...
for f in files
	if successful_retcode(f)
		push!(tests,ConvergenceTest(f,LinearCUDA();
			resume=false,
			...
			atolgoal=1e-8,
			rtolgoal=1e-3,
			path=replace(f,"dt_1e-4" => "dt_1e-4_kxmax_1e-3")))
	end
end

@info "$(length(tests)) converged tests found."

global_logger(make_teelogger(dirname(tests[id].testdatafile),"slurmid="*ENV["SLURM_ARRAY_TASK_ID"]))
